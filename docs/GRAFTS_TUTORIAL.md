...
**Grafts** are shareable memory bundles - the viral engine of Phloem. Export your curated context and share it with others.
...
phloem graft export --tags "architecture,patterns" --output my-arch.graft --name "My Architecture Patterns"
...
2. **Share on Twitter/X**: "Check out my Phloem graft: [link]"
...
phloem graft inspect someone-else.graft

# If it looks good, import it
phloem graft import someone-else.graft
```
...
```bash
phloem graft export --tags "engineering,standards,onboarding" \
...
```bash
phloem graft export --tags "project-x,decisions,architecture" \
...
```bash
phloem graft export --tags "stripe,payments,integration" \
...
```bash
phloem graft export --tags "patterns" --output patterns-v2.graft --name "Patterns v2.0"
```
...
phloem graft export \
  ...
  --output phloem-engineering-standards.graft \
  --name "Phloem Engineering Standards" \
  --desc "Core patterns for Phloem development" \
  --author "Phloem Team"

# 3. Verify
phloem graft inspect phloem-engineering-standards.graft
...
```bash
phloem graft inspect unknown.graft
```
...
Phloem automatically deduplicates on import:
...
phloem graft export --tags "documented" --output with-citations.graft
```
...
```bash
# Last 7 days
phloem graft export --since "7d" --output recent.graft

# Last 2 weeks
phloem graft export --since "336h" --output biweekly.graft
```
...
**Solution**:
1. Check your tags: `phloem status` (shows memory counts)
2. Verify memories have the tags you're filtering by
3. Try without tags to see all memories: `phloem graft export --output all.graft`
...
**Solution**:
1. Inspect first: `phloem graft inspect file.graft`
...
1. **Try the seed graft**: `phloem graft import grafts/phloem-engineering-standards.graft`
2. **Create your first graft**: Export your expertise
3. **Share it**: Help others learn
4. **Build the viral loop**: Every graft shared = more Phloem users

---

**Remember**: Grafts are the viral engine. Every graft you share helps Phloem grow. Share your knowledge!
